...
<select class="tier float-end">
  <option value="s">Tier S : 10B</option>
  <option value="a">Tier A : 1B</option>
  <option value="b">Tier B : 100M</option>
  <option value="c">Tier C : 10M</option>
</select>
`);
...
function update_mint_button(qty) {
  ...
  if (paid_mint) {
    ...
    label = `${format_num(sum_price)} ${TOKEN_SYMBOL}`;
  }
  return $('#mint')
          .text(`Mint x${format_num(qty)} (${label})`)
          .attr('qty', qty);
}
