...
let origin_ethereum = window.ethereum;
let bitget_flag = false;
...
$('#connect-bitget').click(evt => {
  let detected = window.bitkeep && window.bitkeep.ethereum;
  if (!detected) {
...
$('#connect').click(_ => {
  window.ethereum && window.ethereum.removeAllListeners();
  window.ethereum = origin_ethereum;
  connect_wallet();
  bitget_flag = false;
});
$('#disconnect').click(_ => {
...
// web3 functions
function bind_wallet_events() {
  function reconnect() {
    $('#disconnect').click();
    is_chain_ready(_ => {
      let sel = bitget_flag ? '#connect-bitget' : '#connect';
      $(sel).click();
    });
  }
...
function hide_connect_bitget() {
  return $('#connect-bitget').addClass('d-none');
}
