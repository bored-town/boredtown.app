...
function load_balance(addr) {
  ...
  reader.balanceOf(addr).then(raw => {
    let balance = raw2float(raw);
    $('#balance').text(`Balance: ${balance.toLocaleString()} ${TOKEN_SYMBOL}`);
  });
}

// number
function raw2float(raw) {
  let str = ethers.formatUnits(raw.toString(), TOKEN_DECIMALS);
  return Number(str);
}
function float2raw(f) {
  return ethers.parseUnits(f.toString(), TOKEN_DECIMALS);
}
